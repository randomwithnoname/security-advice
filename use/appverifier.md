...
[Obtainium](
https://github.com/randomwithnoname/security-advice/tree/main/use/obtainium.md) will automatically open the share menu to share a newly installed app with AppVerifier if you have it installed and haven't disabled that feature in Obtainium. You only need to verify the initial installation, as Android prevents installing updates that aren't signed by the same key or a key authorized by it, as well as versions older than the one currently installed.
